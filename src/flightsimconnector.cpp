...
bool FlightSimConnector::connected() const { return _connected; }
...
void FlightSimConnector::start() { _connect_fs_timer->start(1000); }
...
void FlightSimConnector::disconnectFS() { FSUIPC_Close(); }
...
void FlightSimConnector::pollData() {
  ...
  if (pollValue<int16_t, 2>(0xE92, wind_dir)) {
    ...
    wind_dir_final =
        wind_dir_final >= 0.0 ? wind_dir_final : 360.0 + wind_dir_final;
  ...
  if (pollValue<int64_t, 8>(0x0560, latitude)) {
    double converted_latitude =
        latitude * 90.0 / (10001750.0 * 65536.0 * 65536.0);
  ...
  if (gs_final > 0.0 && !on_ground) {
    wca_final = qRadiansToDegrees(
        qAsin(qSin(qDegreesToRadians(hdg_final - wind_dir_final)) *
              wind_mag_final / gs_final));
  }
...
bool FlightSimConnector::FlightSimConnector::pollValue(int offset, T &value) {
  DWORD dw_result;
  T tmp_value;
  if (!FSUIPC_Read(offset, n, &tmp_value, &dw_result) ||
      !FSUIPC_Process(&dw_result)) {
